...
export const getCitizen = async (partyId: string, req: { user: User }, api: Pick<ApiService, 'get'> = defaultApi): Promise<CitizenExtended> => {
  const citizenApiBase = getApiBase('citizen');
  const citizenUrl = `${citizenApiBase}/${MUNICIPALITY_ID}/${partyId}`;
  const citizenRes = await api.get<CitizenExtended>({ url: citizenUrl }, req.user).catch(() => null);
...
export const getCitizenPersonnumber = async (
  partyId: string,
  req: { user: User },
  api: Pick<ApiService, 'get'> = defaultApi,
): Promise<CitizenExtended> => {
  const citizenApiBase = getApiBase('citizen');
