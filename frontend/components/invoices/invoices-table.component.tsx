...
export const InvoicesTable: React.FC<{ data: InvoicesData; heading: string; helpText: string }> = (props) => {
          ...
          <Fragment>
            <span className="flex items-center xl:w-[20rem]">
              <Badge
                size="sm"
                variant="solid"
                className={`w-[14px] max-h-[14px] h-[14px] bg-${statusColorMap(item.invoiceStatus.color)} mr-2`}
              />
  ...
  const table = (
    <>
      ...
      ) : props.data?.invoices?.length > 0 ? (
        <></>
      ) : (
        isLoadingInvoices && <p className="px-lg w-3/5">{`Laddar ${props.heading.toLowerCase()}`}</p>
      )}
      {props.data?.invoices?.length > 0 && (
        <div className="px-lg">
