import { CaseStatusResponse } from '@data-contracts/backend/data-contracts';
import { RepresentingMode } from '@interfaces/app';
import { ApiResponse } from '@services/api-service';
import { representingModeDefault } from 'cypress/support/e2e';

export const getCases: (representingMode?: RepresentingMode) => ApiResponse<CaseStatusResponse[]> = (
  representingMode = representingModeDefault
) => ({
  data: [
    {
      caseType: `caseType-Inskickat-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-0',
      id: 'id-0',
      externalStatus: 'Inskickat',
      lastStatusChange: '2024-08-30T12:00:00Z',
      firstSubmitted: '2024-08-25T08:30:00Z',
      isOpenEErrand: true,
      system: 'OPEN_E_PLATFORM',
    },
    {
      caseType: `caseType-Handläggning pågår-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-1',
      id: 'id-1',
      externalStatus: 'Tilldelat för handläggning',
      lastStatusChange: '2024-08-29T11:45:00Z',
      firstSubmitted: '2024-08-25T08:30:00Z',
      isOpenEErrand: true,
      system: 'OPEN_E_PLATFORM',
    },
    {
      caseType: `caseType-Handläggning pågår-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-2',
      id: 'id-2',
      externalStatus: 'Under behandling',
      lastStatusChange: '2024-08-28T09:20:00Z',
      firstSubmitted: '2024-08-25T08:30:00Z',
      isOpenEErrand: true,
      system: 'OPEN_E_PLATFORM',
    },
    {
      caseType: `caseType-Handläggning pågår-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-3',
      id: 'id-3',
      externalStatus: 'Under behandling',
      lastStatusChange: '2024-08-28T09:20:00Z',
      firstSubmitted: '2024-08-25T08:30:00Z',
      isOpenEErrand: true,
      system: 'OPEN_E_PLATFORM',
    },
    {
      caseType: `caseType-Handläggning pågår-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-4',
      id: 'id-4',
      externalStatus: 'Under behandling',
      lastStatusChange: '2024-08-28T09:20:00Z',
      firstSubmitted: '2024-08-25T08:30:00Z',
      isOpenEErrand: true,
      system: 'OPEN_E_PLATFORM',
    },
    {
      caseType: `caseType-Handläggning pågår-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-4',
      id: 'id-4',
      externalStatus: 'Under behandling',
      lastStatusChange: '2024-08-28T09:20:00Z',
      firstSubmitted: '2024-08-25T08:30:00Z',
      isOpenEErrand: true,
      system: 'OPEN_E_PLATFORM',
    },
    {
      caseType: `caseType-Handläggning pågår-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-5',
      id: 'id-5',
      externalStatus: 'Under behandling',
      lastStatusChange: '2024-08-28T09:20:00Z',
      firstSubmitted: '2024-08-25T08:30:00Z',
      isOpenEErrand: true,
      system: 'OPEN_E_PLATFORM',
    },
    {
      caseType: `caseType-Komplettering behövs-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-6',
      id: 'id-6',
      externalStatus: 'Komplettering behövs',
      lastStatusChange: '2024-08-27T14:15:00Z',
      firstSubmitted: '2024-08-25T08:30:00Z',
      isOpenEErrand: true,
      system: 'OPEN_E_PLATFORM',
    },
    {
      caseType: `caseType-Komplettering behövs-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-7',
      id: 'id-7',
      externalStatus: 'Påminnelse om komplettering',
      lastStatusChange: '2024-08-26T16:10:00Z',
      firstSubmitted: '2024-08-25T08:30:00Z',
      isOpenEErrand: true,
      system: 'OPEN_E_PLATFORM',
    },
    {
      caseType: `caseType-Komplettering inkommen, behandling fortsätter-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-8',
      id: 'id-8',
      externalStatus: 'Komplettering inkommen, behandling fortsätter',
      lastStatusChange: '2024-08-25T10:30:00Z',
      firstSubmitted: '2024-08-25T08:30:00Z',
      isOpenEErrand: true,
      system: 'OPEN_E_PLATFORM',
    },
    {
      caseType: `caseType-Beslut finns, se separat information-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-9',
      id: 'id-9',
      externalStatus: 'Beslut finns, se separat information',
      lastStatusChange: '2024-08-24T12:00:00Z',
      firstSubmitted: '2024-08-20T08:30:00Z',
      isOpenEErrand: true,
      system: 'OPEN_E_PLATFORM',
    },
    {
      caseType: `caseType-Väntar på komplettering-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-10',
      id: 'id-10',
      externalStatus: 'Väntar på komplettering',
      lastStatusChange: '2024-08-23T09:45:00Z',
      firstSubmitted: '2024-08-20T08:30:00Z',
      isOpenEErrand: true,
      system: 'OPEN_E_PLATFORM',
    },
    {
      caseType: `caseType-Kompletterad-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-11',
      id: 'id-11',
      externalStatus: 'Kompletterad',
      lastStatusChange: '2024-08-22T13:00:00Z',
      firstSubmitted: '2024-08-20T08:30:00Z',
      isOpenEErrand: true,
      system: 'OPEN_E_PLATFORM',
    },
    {
      caseType: `caseType-Avslutat-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-9',
      id: 'id-9',
      externalStatus: 'Avslutat',
      lastStatusChange: '2024-08-24T12:00:00Z',
      firstSubmitted: '2024-08-20T08:30:00Z',
    },
    {
      caseType: `caseType-Komplettering behövs-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-10',
      id: 'id-10',
      externalStatus: 'Komplettering behövs',
      lastStatusChange: '2024-08-23T09:45:00Z',
      firstSubmitted: '2024-08-20T08:30:00Z',
    },
    {
      caseType: `caseType-Komplettering behövs-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-11',
      id: 'id-11',
      externalStatus: 'Komplettering behövs',
      lastStatusChange: '2024-08-22T13:00:00Z',
      firstSubmitted: '2024-08-20T08:30:00Z',
    },
    {
      caseType: `caseType-AvslutatTest-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-12',
      id: 'id-12',
      externalStatus: 'Klart',
      lastStatusChange: '2024-08-21T15:30:00Z',
      firstSubmitted: '2024-08-15T08:30:00Z',
      isOpenEErrand: true,
      system: 'OPEN_E_PLATFORM',
    },
    {
      caseType: `caseType-Avslutat-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-13',
      id: 'id-13',
      externalStatus: 'Avslutat',
      lastStatusChange: '2024-08-20T14:45:00Z',
      firstSubmitted: '2024-08-15T08:30:00Z',
      isOpenEErrand: true,
      system: 'OPEN_E_PLATFORM',
    },
    {
      caseType: `caseType-Avslutat-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-14',
      id: 'id-14',
      externalStatus: 'Ärendet arkiveras',
      lastStatusChange: '2024-08-19T11:15:00Z',
      firstSubmitted: '2024-08-15T08:30:00Z',
      isOpenEErrand: false,
      system: 'SUPPORT_MANAGEMENT',
    },
    {
      caseType: `caseType-Komplettering behövs-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-15',
      id: 'id-15',
      externalStatus: 'Komplettering behövs',
      lastStatusChange: '2024-08-18T17:30:00Z',
      firstSubmitted: '2024-08-15T08:30:00Z',
      isOpenEErrand: false,
      system: 'SUPPORT_MANAGEMENT',
    },

    {
      caseType: `caseType-Komplettering behövs-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-15',
      id: 'id-15',
      externalStatus: 'Komplettering behövs',
      lastStatusChange: '2024-08-18T17:30:00Z',
      firstSubmitted: '2024-08-15T08:30:00Z',
      isOpenEErrand: false,
      system: 'SUPPORT_MANAGEMENT',
    },
    {
      caseType: `caseType-Komplettering behövs-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-15',
      id: 'id-15',
      externalStatus: 'Komplettering behövs',
      lastStatusChange: '2024-08-18T17:30:00Z',
      firstSubmitted: '2024-08-15T08:30:00Z',
      isOpenEErrand: false,
      system: 'SUPPORT_MANAGEMENT',
    },
    {
      caseType: `caseType-Komplettering behövs-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-15',
      id: 'id-15',
      externalStatus: 'Komplettering behövs',
      lastStatusChange: '2024-08-18T17:30:00Z',
      firstSubmitted: '2024-08-15T08:30:00Z',
      isOpenEErrand: false,
      system: 'SUPPORT_MANAGEMENT',
    },
    {
      caseType: `caseType-Komplettering behövs-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-15',
      id: 'id-15',
      externalStatus: 'Komplettering behövs',
      lastStatusChange: '2024-08-18T17:30:00Z',
      firstSubmitted: '2024-08-15T08:30:00Z',
      isOpenEErrand: false,
      system: 'SUPPORT_MANAGEMENT',
    },
    {
      caseType: `caseType-Handläggning pågår-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-15',
      id: 'id-15',
      externalStatus: 'Handläggning pågår',
      lastStatusChange: '2024-08-18T17:30:00Z',
      firstSubmitted: '2024-08-15T08:30:00Z',
      isOpenEErrand: false,
      system: 'SUPPORT_MANAGEMENT',
    },
    {
      caseType: `caseType-Komplettering behövs-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-15',
      id: 'id-15',
      externalStatus: 'Komplettering behövs',
      lastStatusChange: '2024-08-18T17:30:00Z',
      firstSubmitted: '2024-08-15T08:30:00Z',
      isOpenEErrand: false,
      system: 'SUPPORT_MANAGEMENT',
    },
    {
      caseType: `caseType-Komplettering behövs-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-15',
      id: 'id-15',
      externalStatus: 'Komplettering behövs',
      lastStatusChange: '2024-08-18T17:30:00Z',
      firstSubmitted: '2024-08-15T08:30:00Z',
      isOpenEErrand: false,
      system: 'SUPPORT_MANAGEMENT',
    },
    {
      caseType: `caseType-Komplettering behövs-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-15',
      id: 'id-15',
      externalStatus: 'Komplettering behövs',
      lastStatusChange: '2024-08-18T17:30:00Z',
      firstSubmitted: '2024-08-15T08:30:00Z',
      isOpenEErrand: false,
      system: 'SUPPORT_MANAGEMENT',
    },
    {
      caseType: `caseType-Handläggning pågår-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-15',
      id: 'id-15',
      externalStatus: 'Handläggning pågår',
      lastStatusChange: '2024-08-18T17:30:00Z',
      firstSubmitted: '2024-08-15T08:30:00Z',
      isOpenEErrand: false,
      system: 'SUPPORT_MANAGEMENT',
    },
    {
      caseType: `caseType-Avslutat-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-16',
      id: 'id-15',
      externalStatus: 'Avslutat',
      lastStatusChange: '2024-08-18T17:30:00Z',
      firstSubmitted: '2024-08-15T08:30:00Z',
      isOpenEErrand: false,
      system: 'SUPPORT_MANAGEMENT',
    },
    {
      caseType: `caseType-Avslutat-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-17',
      id: 'id-15',
      externalStatus: 'Avslutat',
      lastStatusChange: '2024-08-18T17:30:00Z',
      firstSubmitted: '2024-08-15T08:30:00Z',
      isOpenEErrand: false,
      system: 'SUPPORT_MANAGEMENT',
    },
    {
      caseType: `caseType-Avslutat-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-18',
      id: 'id-15',
      externalStatus: 'Avslutat',
      lastStatusChange: '2024-08-18T17:30:00Z',
      firstSubmitted: '2024-08-15T08:30:00Z',
      isOpenEErrand: false,
      system: 'SUPPORT_MANAGEMENT',
    },
    {
      caseType: `caseType-Sparat-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-19',
      id: 'id-19',
      externalStatus: 'Sparat',
      lastStatusChange: '2024-08-17T10:00:00Z',
      isOpenEErrand: true,
      system: 'OPEN_E_PLATFORM',
    },
    {
      caseType: `caseType-Väntar på flerpartssignering-${RepresentingMode[representingMode]}`,
      caseId: 'caseId-20',
      id: 'id-20',
      externalStatus: 'Väntar på flerpartssignering',
      lastStatusChange: '2024-08-17T09:00:00Z',
      isOpenEErrand: true,
      system: 'OPEN_E_PLATFORM',
    },
  ],
  message: 'success',
});
