...
export const handleCaseResponse: (res: ApiResponse<CaseResponse[]>) => ICase[] = (res) =>
  res.data.map((n: CaseResponse) => ({
...
export const getCases: () => Promise<CasesData> = () =>
  apiService
    .get<ApiResponse<CaseResponse[]>>('cases')
    .then((res) => ({ cases: handleCaseResponse(res.data), labels: [] } as CasesData))
    .catch((e) => ({ ...emptyCaseList, error: e.response?.status ?? 'UNKNOWN ERROR' } as CasesData));
