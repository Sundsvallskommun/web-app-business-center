    ...
    <>
      <div className="flex flex-col gap-24">
        ...
        <h1 className="text-h2-sm desktop:text-h2-lg mb-0 break-word hyphens-auto">
          {t('decisions:parkingPermit.lost.success.title')}
        </h1>
        <div>
          <p className="mt-0">{t('decisions:parkingPermit.lost.success.description')}</p>
          <p className="mt-0">
            {t('decisions:parkingPermit.lost.success.processing')}
          </p>
        </div>
      </div>
      <div className="flex flex-col desktop:flex-row gap-x-24 gap-y-20 desktop:items-center mt-40">
        <Button size="lg" color="vattjom" rightIcon={<ArrowRight />} onClick={() => setIsEditing(null)}>
          {t('decisions:parkingPermit.lost.success.backToOverview')}
        </Button>
      </div>
    </>
  );
